...
try {
  ...
  const read = path => productionSchema(JSON.parse(fs.readFileSync(path, "utf8")), false);
  const expected = read(process.argv[2]);
  const actual = read(process.argv[3]);
  if (JSON.stringify(expected) !== JSON.stringify(actual)) {
    console.error("production schema drifted from the reviewed baseline; aborting migration", { expected, actual });
    process.exit(1);
  }
} catch (error) {
