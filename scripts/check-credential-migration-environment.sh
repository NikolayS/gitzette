...
environment="$(gh api "repos/$repository/environments/credential-migration")"
...
[[ "$actual" == "$expected" ]] || { echo "credential-migration environment differs from reviewed policy" >&2; exit 1; }
