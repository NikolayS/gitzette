...
test('feature restoration queues exactly one owner edition, preserves old publication and is idempotent',async()=>{
 ...
 expect(db.query("SELECT week_key,status FROM generation_jobs WHERE id='fa29eb25-88ba-40c7-8fce-ed7136754f8b'").get()).toEqual({week_key:'2026-W36',status:'queued'});
...
test('feature restoration respects missing owner, suppression and current work',async()=>{
 ...
 db.exec(repair);
 expect(db.query("SELECT COUNT(*) n FROM generation_jobs WHERE id='fa29eb25-88ba-40c7-8fce-ed7136754f8b'").get()).toEqual({n:0});
 }finally{db.close();}
 }
});
