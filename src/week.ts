...
export const FIRST_SUPPORTED_ISO_WEEK_YEAR = 2026;
...
export function parseIsoWeekKey(value: string, now = new Date()): IsoWeek {
  const match = WEEK_KEY.exec(value);
  if (!match) throw new Error("invalid ISO week");
  const year = Number(match[1]);
  if (year < FIRST_SUPPORTED_ISO_WEEK_YEAR || year > now.getUTCFullYear() + 1) {
    throw new Error("ISO week year is outside the supported operating range");
  }
...
export function isCompletedIsoWeekKey(value: string, now = new Date()): boolean {
  try {
    const parsed = parseIsoWeekKey(value, now);
