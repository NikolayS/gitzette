...
function weekNavBar(username: string, week_key: string): string {
  ...
  return `<span style="font-family:'IBM Plex Mono',monospace;font-size:11px;letter-spacing:.04em;display:flex;align-items:center;gap:8px;white-space:nowrap;">
    <a href="/${username}/${prev}" style="color:#f7f4ee;border:none;text-decoration:none;opacity:.85;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity='.85'">← ${short(prev)}</a>
    <span style="color:#555;">|</span>
    <span style="font-weight:600;color:#f7f4ee;">${short(week_key)}</span>
    <span style="color:#555;">|</span>
    ${isFuture
      ? `<span style="color:#555;">${short(next)} →</span>`
      : `<a href="/${username}/${next}" style="color:#f7f4ee;border:none;text-decoration:none;opacity:.85;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity='.85'">${short(next)} →</a>`
    }
  </span>`;
}
...
<body>
  ...
  <div style="background:#0f0f0f;padding:8px 16px;display:flex;align-items:center;justify-content:space-between;font-family:'IBM Plex Mono',monospace;font-size:12px;gap:12px;flex-wrap:wrap;">
    <a href="/${username}" style="color:#f7f4ee;border:none;text-decoration:none;">@${username}</a>
    ${weekNavBar(username, dispatch.week_key)}
  </div>`}
  <div class="paper">
    <div class="header">
